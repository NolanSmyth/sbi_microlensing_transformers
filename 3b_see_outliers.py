...
def plot_light_curve_and_residuals(
    x_event, theta_true, theta_recovered, ax_lc, ax_res, case_idx
):
    """
    ...
    try:
        # True model
        theta_true_np = (
            theta_true.cpu().numpy() if torch.is_tensor(theta_true) else theta_true
        )
        mag_true = simulate_microlensing_event_pytorch(
            torch.tensor(theta_true_np), torch.tensor(t_dense)
        ).numpy()

        # Recovered model
        theta_rec_np = (
            theta_recovered
            if not torch.is_tensor(theta_recovered)
            else theta_recovered.cpu().numpy()
        )
        mag_recovered = simulate_microlensing_event_pytorch(
            torch.tensor(theta_rec_np), torch.tensor(t_dense)
        ).numpy()
    ...
    # Plot light curve
    ax_lc.scatter(
        times_original, mags_valid, c="black", s=10, alpha=0.7, label="Observed"
    )
...
def main():
    ...
    with torch.no_grad():
        for j, i in enumerate(trange(kept, desc="Sample", leave=False)):
            ...
            samples_t = posterior.sample(
                (NUM_POST_SAMPLES,), x=x_i, show_progress_bars=SHOW_PROGRESS_BARS
            )
            ...
            score = compute_outlier_score(
                true_thetas_np[j], map_params, s_np, OUTLIER_METRIC
            )
    ...
    # --- Find worst cases -----------------------------------------------------------
    worst_indices = np.argsort(outlier_scores)[-NUM_WORST_CASES:][
        ::-1
    ...
    # --- Plot worst cases -----------------------------------------------------------
    param_names = (
        PARAM_NAMES if "PARAM_NAMES" in globals() else [f"θ{d}" for d in range(D)]
    )
    ...
    for i, worst_idx in enumerate(worst_indices):
        ...
        plot_light_curve_and_residuals(
            x_event, true_theta, recovered_theta, ax_lc, ax_res, original_idx
        )
        ...
        ax_params.errorbar(
            x_pos - 0.1, true_vals, fmt="ro", markersize=8, label="True", capsize=3
        )
        ax_params.errorbar(
            x_pos + 0.1,
            rec_vals,
        ...
        ax_params.set_xticklabels(
            [name.replace("log10_", "") for name in param_names],
            rotation=45,
            ha="right",
        )
    ...
    out_path = os.path.join(
        FIGURES_PATH, f"worst_cases_{OUTLIER_METRIC}_snr{MIN_PEAK_SNR:.1f}.png"
    )
    ...
    print(
        f"Score range: {outlier_scores[worst_indices[-1]]:.4f} to {outlier_scores[worst_indices[0]]:.4f}"
    )

    print(
        "tE of worst cases:"
        + ", ".join(
            f"{10**thetas[kept_idx[idx],2].item():.2f}" for idx in worst_indices
        )
    )
