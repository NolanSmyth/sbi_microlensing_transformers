...
from utils import simulate_microlensing_event_pytorch, ensure_dir, generate_observation_times
...
def create_schematic_figure_publication():
    ...
    ax1.scatter(param_0_samples, param_1_samples, s=2, alpha=0.5, color='C0', rasterized=False)
    ...
    flux_ideal = simulate_microlensing_event_pytorch(
        true_params, times_ideal, phot_err=0.0
    ).cpu().numpy()
    ...
    flux_augmented = simulate_microlensing_event_pytorch(
    ...
    ax2.plot(times_ideal, flux_ideal, color='grey', ls='--', lw=1, alpha=0.7, label='Ideal Model')
    ax2.errorbar(times_augmented, flux_augmented, yerr=phot_err_augmented, fmt='.', color='C0', ms=4, label='Observation', rasterized=False)
    ...
    mean = [0, 0]
    cov = [[1.2, -0.8], [-0.8, 0.8]]
    x_grid, y_grid = np.mgrid[-2.5:2.5:.01, -2:2:.01]
