...
def main():
    ...
    xs_unpadded = torch.stack([generate_base_simulation(theta) for theta in thetas_overfit])
    ...
    assert final_loss < -5.0, "Overfitting Test FAILED: Final loss is not low enough. Model may not be learning."
    print("Overfitting Test PASSED: Model successfully minimized loss on a single batch.")
