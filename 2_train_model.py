...
from config import *
from utils import MicrolensingAugmentationDataset, ensure_dir, set_global_seeds, save_posterior_bundle, load_saved_cpu, RecoverCfg
...
def _make_scheduler(optim, t_max=None):
    ...
    if LR_SCHEDULER["name"] == "cosine":
        return CosineAnnealingLR(optim,
                                 T_max=t_max or MAX_EPOCHS,
...
# ----------------------------- training loop --------------------------- #
def train_density_estimator(density_estimator, train_loader, val_loader,
                            max_epochs, patience, lr, device, experiment,
                            on_improve=None, lr_embed=None, freeze_embed=False):
    ...
    if hasattr(density_estimator, "embedding_net") and split:
        ...
        rest_params = [p for p in density_estimator.parameters() if id(p) not in embed_ids]
        ...
        if freeze_embed:
            ...
            param_groups = [
                ...
                {"params": embed_params, "lr": (lr_embed if lr_embed is not None else lr * 0.25)},
            ]
    ...
    for epoch in range(max_epochs):
        ...
        for xb, thetab in train_loader:
            xb, thetab = xb.to(device, non_blocking=True), thetab.to(device, non_blocking=True)
            optim.zero_grad(set_to_none=True)
            loss = density_estimator.loss(thetab, condition=xb).mean()
            if not torch.isfinite(loss):
                print("⚠️  non-finite loss – skip batch"); continue
            loss.backward()
        ...
        with torch.no_grad():
            for xb, thetab in val_loader:
                xb, thetab = xb.to(device, non_blocking=True), thetab.to(device, non_blocking=True)
                running += density_estimator.loss(thetab, condition=xb).mean().item() * xb.size(0)
        val_NLL = running / len(val_loader.dataset)

        # ---- scheduler / logging ------------------------------------- #
        if sched:
            sched.step(val_NLL) if isinstance(sched, ReduceLROnPlateau) else sched.step()
            experiment.log_metric("learning_rate", optim.param_groups[0]["lr"], step=epoch)
        ...
        if epoch == 0 or (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1:4d} | train NLL {train_NLL:.3f} | "
...
def main():
    ...
    experiment.log_parameters(
        {k: globals()[k] for k in [
            "TOTAL_DURATION","MAX_NUM_POINTS","PHOT_ERR_MIN","PHOT_ERR_MAX",
            "AUG_NUM_GAPS_RANGE","AUG_GAP_LENGTH_RANGE","AUG_DROPOUT_RATE_RANGE",
            "NUM_AUGMENTATIONS_PER_SIM","D_MODEL","N_HEAD","D_HID","N_LAYERS",
            "DROPOUT"
        ]}
    )
    experiment.log_parameters({
        "BATCH_SIZE": args.batch_size, "MAX_EPOCHS": args.max_epochs,
        "PATIENCE": args.patience, "LEARNING_RATE": args.lr, "SEED": args.seed
    })

    RECOVER_CFG = RecoverCfg()  # or RecoverCfg(n_peak_min=2, base_k=3.0, ...)
    experiment.log_parameters({f"recover_cfg.{k}": v for k, v in asdict(RECOVER_CFG).items()})

    print({"TOTAL_DURATION": TOTAL_DURATION, "MAX_NUM_POINTS": MAX_NUM_POINTS,
       "BATCH_SIZE": args.batch_size, "MAX_EPOCHS": args.max_epochs,
    ...
    dataset = MicrolensingAugmentationDataset(
        thetas=base_thetas, n_augs=NUM_AUGMENTATIONS_PER_SIM,
    )
    print("Virtual dataset length:", len(dataset))

    embed = TransformerEmbeddingNet(
        d_model=D_MODEL, nhead=N_HEAD, d_hid=D_HID,
        nlayers=N_LAYERS, dropout=DROPOUT, input_dim=3
    ).to(device)

    builder = posterior_nn(model="maf", embedding_net=embed,
    ...
    if args.resume is not None:
        ...
        try:
            sd = saved.get("flow_state_dict", None) or saved["density_estimator"].state_dict()
        ...
        if args.freeze_embed and hasattr(flow, "embedding_net"):
            for p in flow.embedding_net.parameters(): p.requires_grad = False
            print("[resume] embedding frozen.")

    # ---- save bundle (robust) ----
    embed_cfg = {
        "D_MODEL": D_MODEL, "N_HEAD": N_HEAD, "D_HID": D_HID,
        "N_LAYERS": N_LAYERS, "DROPOUT": DROPOUT, "INPUT_DIM": 3
    }
    ...
    def _save_best(state_dict, epoch, val):
        ...
        save_posterior_bundle(
            flow, PRIOR, embed_cfg, tmp_path,
            meta={"checkpoint": True, "best_val": float(val), "epoch": int(epoch), "recover_cfg": asdict(RECOVER_CFG)}
        )
        os.replace(tmp_path, TRAINED_POSTERIOR_PATH)  # atomic rename
        print(f"[checkpoint] saved best @ epoch {epoch} (val={val:.4f}) → {TRAINED_POSTERIOR_PATH}")

    train_loader, val_loader = build_dataloaders(dataset, args.batch_size, VALIDATION_FRACTION)

    print("Starting explicit training loop …")
    flow, best_val = train_density_estimator(
        flow, train_loader, val_loader,
        args.max_epochs, args.patience, args.lr, device, experiment,
        on_improve=_save_best,
        lr_embed=args.lr_embed, freeze_embed=args.freeze_embed
    )
    experiment.log_metric("best_validation_loss", best_val)


    save_posterior_bundle(
    flow, PRIOR, embed_cfg, TRAINED_POSTERIOR_PATH,
    meta={"checkpoint": False,
          "best_val": float(best_val),
          "train_args": {"batch_size": args.batch_size, "max_epochs": args.max_epochs,
                        "patience": args.patience, "lr": args.lr, "seed": args.seed}, "recover_cfg": asdict(RECOVER_CFG)}
)
