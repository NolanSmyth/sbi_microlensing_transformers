...
def prepare_data_for_network(times, flux, device, errors=None):
    ...
    data = torch.stack((normalized_times, flux.to(dtype=torch.float32, device="cpu"), sigma_vec), dim=1)
...
def check_mcmc_convergence(sampler, min_samples=100, tolerance=0.01, quiet=False):
    ...
    try:
        ...
        if chain_length < min_samples:
            if not quiet:
                print(f"Not enough samples for convergence check (need {min_samples}, have {chain_length})")
            return False, None, None
        
        # Calculate autocorrelation time
        try:
            tau = sampler.get_autocorr_time(tol=0)  # tol=0 to avoid warnings for short chains
        except emcee.autocorr.AutocorrError:
            if not quiet:
                print("Warning: Autocorrelation time estimation failed (chains too short)")
            return False, None, None
        ...
        effective_samples = nwalkers*chain_length / (2.0 * tau)  # Factor of 2 for conservative estimate
        ...
        if not quiet:
            ...
            for i, param_name in enumerate(PARAM_NAMES):
                ...
                print(f"  {param_name:>15}: tau={tau[i]:6.1f}, ratio={ratio:6.1f}, eff_samples={effective_samples[i]:6.1f} {status}")
...
def run_mcmc_sampling(times, flux, flux_err, nwalkers=32, nsteps=5000, burn_in=1000, initial_params=None):
    """
    ...
    for i in range(0, nsteps, check_interval):
        ...
        if i > 0 and (i + remaining_steps) >= nsteps // 2:  # Only check after halfway point
...
def create_lightcurve_comparison_plot(npe_samples, mcmc_samples, true_params, 
                                    observation_times, flux, noise_level, save_path):
    """
    ...
    npe_lightcurves = []
    npe_subset = npe_samples[::max(1, len(npe_samples)//200)]  # Use every nth sample, max 200
    ...
    mcmc_lightcurves = []
    mcmc_subset = mcmc_samples[::max(1, len(mcmc_samples)//200)]  # Use every nth sample, max 200
    ...
    ax.errorbar(observation_times, flux.cpu(), yerr=noise_level,
                fmt='o', color='black', alpha=0.7, capsize=4, markersize=5,
                label='Data', zorder=0)
    ...
    ax.fill_between(smooth_times, npe_lower, npe_upper, 
                    color='#0072B2', alpha=0.3)
    ax.plot(smooth_times, npe_median, 
            color='#0072B2', linewidth=3, label='NPE', zorder=2)
    
    # Plot MCMC percentile bands  
    ax.fill_between(smooth_times, mcmc_lower, mcmc_upper,
                    color='#D55E00', alpha=0.3)
    ax.plot(smooth_times, mcmc_median,
            color='#D55E00', linewidth=3, label='MCMC', zorder=2, ls='--')
...
def create_corner_overlay_plot(npe_samples, mcmc_samples, true_params, noise_level, save_path):
    """
    ...
    ranges_auto = [(max(a, lo), min(b, hi)) for (a,b), (lo,hi) in zip(zip(mins, maxs), ranges)]
...
def create_trace_plots(sampler, true_params, burn_in, save_path):
    ...
    for i in range(ndim):
        ...
        # Mark true value
        ax.axhline(true_params[i], color='green', linestyle='-', linewidth=2, alpha=0.8, label='True value')
...
def main():
    ...
    observation_times = generate_observation_times(num_gaps_range=(2, 2), 
                                                 gap_length_range=(1, 1.5), 
                                                 dropout_rate_range=(0.4, 0.4))
    ...
    for noise in noise_levels:
        ...
        data_net = prepare_data_for_network(observation_times, flux, device, errors=noise)

        npe_start = time.time()
        npe_samples = posterior.sample((NUM_NPE_SAMPLES,), x=data_net, show_progress_bars=True)
        ...
        initial_params = true_params_test * (1 + 0.01 * np.random.randn(N_WALKERS, len(true_params_test)))

        mcmc_samples, mcmc_time, sampler = run_mcmc_sampling(
            observation_times, flux, noise, 
            nwalkers=N_WALKERS, nsteps=NUM_MCMC_SAMPLES, burn_in=BURN_IN, initial_params=initial_params
        )
        ...
        create_lightcurve_comparison_plot(
            npe_samples.cpu().numpy(), mcmc_samples,
            true_params_test.cpu().numpy(), observation_times, flux, noise,
            lightcurve_path
        )
        ...
        create_corner_overlay_plot(
            npe_samples.cpu().numpy(), mcmc_samples,
            true_params_test.cpu().numpy(), noise, corner_overlay_path
        )
    ...
    create_trace_plots(sampler, true_params_test.cpu().numpy(), BURN_IN, 
